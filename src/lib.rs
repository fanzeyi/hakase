...
mod tests {
    ...
    fn create_test_server(password: Option<String>) -> TestServer {
        let config = Config::new(
            password,
            env::var("DATABASE_URL").unwrap_or_else(|_| ":memory:".to_string()),
        );
        let app_state = AppState::new(config);
        let app = create_router(app_state);
    ...
    async fn test_lookup() {
        ...
        let response = server.post("/create").form(&[("url", url)]).await;
        ...
        let response = server.get(&format!("{}~", location)).await;
